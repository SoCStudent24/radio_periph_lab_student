...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    printf("Calculate number of Bytes: 4 bytes are read 2048 times for a total of 4*2048 = 8192\n");
    float bytes_transferred = 8192.0;

    //Calculate time (stop_time-start_time at 125 MHz to #seconds)
    float time_spent = (stop_time-start_time) / (125000000.0);
    printf("Calculate time: Clock Cycles / (125MHz) = %f \n", time_spent);
