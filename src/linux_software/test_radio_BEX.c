...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float bytes_transferred = (stop_time-start_time) * 4;
    printf("Calculate number of Bytes: 4 bytes * Clock Cycles = %f\n",bytes_transferred);
    

    //Calculate time (stop_time-start_time at 48 kHz to #seconds)
    float time_spent = (stop_time-start_time) / (48000.0);
    printf("Calculate time: Clock Cycles / (48 kHz) = %f \n", time_spent);

    //Calculate Mbytes/second
    throughput= bytes_transferred/(time_spent* 1000000);
    printf("Calculate Mbytes/second: %f\n",throughput);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Bex G - Custom Peripheral Demonstration\n\r");
